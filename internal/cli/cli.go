...
func NewApp(version string, start time.Time) (app *cli.App) {
	...
	app.Commands = cli.CommandsByName{
		...
		{
			Name:  "kv",
			Usage: "perform actions on kv configuration (locally)",
			Subcommands: cli.CommandsByName{
				{
					Name:      "get-path",
					Usage:     "display the currently used vault KV path in the statefile",
					ArgsUsage: " ",
					Action:    cmd.ExecWrapper(cmd.KVGetPath),
				},
				{
					Name:      "set-path",
					Usage:     "update the vault secret KV path in the statefile",
					ArgsUsage: "<path>",
					Action:    cmd.ExecWrapper(cmd.KVSetPath),
				},
				{
					Name:      "get-version",
					Usage:     "display the currently used vault KV version in the statefile",
					ArgsUsage: " ",
					Action:    cmd.ExecWrapper(cmd.KVGetVersion),
				},
				{
					Name:      "set-version",
					Usage:     "update the vault KV version in the statefile",
					ArgsUsage: "<version>",
					Action:    cmd.ExecWrapper(cmd.KVSetVersion),
				},
			},
		},
		{
